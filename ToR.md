...
**Possible** | Communication problem between WANO PI staff. This
question may be settled by WANO top management | Server down. It is
risk with low probability and low impact. The server unavailable in
couple of hours or even days is not critical. When server will up all
the data can be synchronise (from INPO server duration test operation)
or restore (if need) from local copy. | WANO LO image lost risk has
low probability but high impact so we have to start a ‘commercial’ DB
