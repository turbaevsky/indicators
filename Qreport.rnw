...
<<ld>>=
lastDate <- 201606
dateRange <- c(200712,200812,200912,201012,201112,201212,201312,201412,201512,201606)
...
'Percentage Achieving Individual Target (Objective 100 percent)',
'Percentage Achieving Industry Worst Quartile Target (Objective 75 percent)')
...
for (rcNum in 1:4)
	...
	cat('\\multicolumn{2}{|c||}{\\textbf{',rcs[rcNum],'}}','&',
		...
		'\\\\\n\\hline','\n')
	cat('Key Indicator','&',
		'Number of units with qualified results','&',
		'Number of units achieving individual target','&',
		'Percentage of regional centre units achieving individual target','&',
		'Number of units achieving industry worst quartile target','&',
		'Percentage of regional centre units achieving industry worst quartile target',
		'\\\\\n\\hline','\n')
	for (i in 1:5)
		{
		l <- i+(rcNum-1)*5
...
%------------------------------------------------------------
<<IdvCharts, fig=TRUE, height=5, width=6>>=
<<ld>>
par(mfrow=c(2,3))
#library(plotrix)
...
<<IdsCharts, fig=TRUE, height=5, width=6>>=
<<ld>>
ind <- c('FLR','CRE','TISA','US7','SSPI')
par(mfrow=c(2,3))
for (i in 1:5)
...
<<PerfTrendWW, fig=TRUE, height=11, width=6>>=
<<PerfTrendCalc>>
par(mfrow=c(5,2))
for (i in 1:10)
	{
	if (i%%2) lim <- 100 else lim <- 75
...
<<PerfTrendAC, fig=TRUE, height=11, width=6>>=
<<PerfTrendCalc>>
par(mfrow=c(5,2))
for (i in 1:10)
	{
...
<<PerfTrendMC, fig=TRUE, height=11, width=6>>=
<<PerfTrendCalc>>
par(mfrow=c(5,2))
for (i in 1:10)
	{
...
<<PerfTrendPC, fig=TRUE, height=11, width=6>>=
<<PerfTrendCalc>>
par(mfrow=c(5,2))
for (i in 1:10)
	{
...
<<PerfTrendTC, fig=TRUE, height=11, width=6>>=
<<PerfTrendCalc>>
par(mfrow=c(5,2))
for (i in 1:10)
	{
...
The LTT-2020 below should be met by the end of 2020, i.e., the unit and industry indicator values should be less than or equal to the values defined below for each indicator.
...
\hline
Safety System Performance Indicator (SSPI) & unavailability & SP1 and SP2: 0.020 & 100 percent of worldwide units achieve the individual targets\\
&&SP5 (EAC): 0.025&\\
\hline
...
\subsubsection*{Forced Loss Rate (FLR)}
...
Planned energy losses are those corresponding to outages or power reductions which were planned and scheduled at least 4 weeks in advance (see clarifying notes for exceptions).
